...
from edgepilot.execution.environment import credential_options
...
from edgepilot.execution.run_state import EMERGENCY_STOP_FILE
from edgepilot.execution.run_state import RUNTIME_FILE
...
def _positions_report_with_unrealized_pnl(
    node: TradingNode,
) -> tuple[Any, list[str]]:
    """Add mark-to-market P&L to the native position report when quotes exist."""
    report = node.trader.generate_positions_report()
    if report is None or report.empty:
        return report, []

    unrealized_by_position: dict[str, str] = {}
    errors: list[str] = []
    for position in node.cache.positions_open():
        ...
        if unrealized_pnl is not None:
            unrealized_by_position[str(position.id)] = str(unrealized_pnl)

    report = report.copy()
    report["unrealized_pnl"] = [
        unrealized_by_position.get(str(position_id))
        for position_id in report.index
    ]
...
def _write_runtime_snapshot(
    ...
    try:
        positions_report, position_errors = _positions_report_with_unrealized_pnl(node)
        reports["positions"] = _frame_rows(positions_report)
...
def execute_trading(
    *,
    ...
    for venue_record in venue_records:
        ...
        # setting applies, so the websocket transport tunnels through the same
        # proxy the REST client already picks up from the environment.
        apply_proxy_url(data_options, adapter.data_config_path)
