"""Command-line composition root for EdgePilot Live."""
...
from edgepilot.marketplace_client import client as marketplace
from edgepilot.execution.run_state import load_run
from edgepilot.execution.run_state import load_execution
from edgepilot.execution.run_state import record_execution_result
...
strategies_root = _lazy_callable("edgepilot.strategies.discovery", "strategies_root")
strategy_names = _lazy_callable("edgepilot.strategies.discovery", "strategy_names")
credential_requirements = _lazy_callable("edgepilot.execution.environment", "credential_requirements")
load_preset = _lazy_callable("edgepilot.strategies.presets", "load_preset")
preset_backtest_values = _lazy_callable("edgepilot.strategies.presets", "preset_backtest_values")
preset_markets = _lazy_callable("edgepilot.strategies.presets", "preset_markets")
preset_names = _lazy_callable("edgepilot.strategies.presets", "preset_names")
...
def _trade(args: argparse.Namespace, mode: str) -> int:
    ...
    with marketplace.strategy_operation_lock(strategy.name):
        ...
        claim_trading_slot(
            runs_path,
            run_id,
            new_record=record if not args.run_id else None,
        )
        if not args.run_id:
            print(f"Run: {run_id}")
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "strategies":
            ...
            from edgepilot.service.local_service import ensure_service
