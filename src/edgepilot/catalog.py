...
UTC = timezone.utc
LOGGER = logging.getLogger("edgepilot.catalog")
...
def pull_data(
    *,
    ...
    node = BacktestNode([])
    factory_cls = resolve_path(adapter.data_factory_path)
    ...
    *,
    ...
    with tqdm(total=expected_total, unit="bar", desc=f"OKX {interval} bars") as progress:
        for interval_start, interval_end in intervals:
            ...
            start_cursor = pd.Timestamp(interval_start - interval)
            end_cursor = pd.Timestamp(interval_end + interval)
            pages: list[list[Bar]] = []
...
async def _pull_binance_bars(
    *,
    ...
    for bar in downloaded:
        ...
        if close_ns in existing_events:
            continue
