"""Authenticated Marketplace catalog and strategy package installer."""
...
def _guest_request(
        path: str, query: dict[str, str] | None = None, *, method: str = "GET",
        payload: dict[str, Any] | None = None, structured_errors: bool = False) -> bytes:
    request_path = path
    if query:
        request_path = f"{path}?{urlencode({key: value for key, value in query.items() if value})}"
    body = None if payload is None else json.dumps(payload).encode("utf-8")
    headers = {"Accept": "application/json", "User-Agent": "EdgePilot Marketplace/1.0"}
    if body is not None:
        headers["Content-Type"] = "application/json"
    request = Request(
        f"{marketplace_origin()}{request_path}",
        data=body,
        method=method,
        headers=headers,
    )
    try:
        ...
        try:
            ...
            raise RuntimeError("Marketplace request failed.") from parse_error
        code = payload.get("error", {}).get("code") if isinstance(payload.get("error"), dict) else payload.get("error")
        if structured_errors:
            error = payload.get("error", {}) if isinstance(payload.get("error"), dict) else {}
            raise MarketplaceRequestError(
                str(code or exc.code), exc.code, code in _RETRYABLE_MARKETPLACE_CODES,
                retry_after=_retry_after(exc.headers),
                limiting_scopes=error.get("limiting_scopes") if isinstance(error.get("limiting_scopes"), list) else None,
                quotas=error.get("quotas") if isinstance(error.get("quotas"), dict) else None,
            ) from exc
        raise RuntimeError(f"Marketplace request failed ({code or exc.code}).") from exc
    except URLError as exc:
        raise RuntimeError("Marketplace request failed (SERVICE_UNAVAILABLE).") from exc
...
def guest_search(*, query: str = "", asset: str = "", venue: str = "", category: str = "", data_type: str = "", risk_profile: str = "", min_capacity_usd: float | None = None, sort: str = "published", locale: str = "", page: int = 1, page_size: int = 30) -> dict[str, Any]:
    normalized_risk_profile = RISK_PROFILE_ALIASES.get(risk_profile.strip().lower(), risk_profile.strip().lower())
    if not isinstance(page, int) or isinstance(page, bool) or page < 1:
        raise ValueError("Marketplace page must be a positive integer")
    if not isinstance(page_size, int) or isinstance(page_size, bool) or page_size < 1 or page_size > 100:
        raise ValueError("Marketplace page size must be between 1 and 100")
    payload = _guest_request("/api/live/guest/strategies", {"q": query, "asset": asset, "venue": venue, "category": category, "data_type": data_type, "risk_profile": normalized_risk_profile, "min_capacity_usd": "" if min_capacity_usd is None else str(min_capacity_usd), "sort": sort, "locale": locale, "page": str(page), "page_size": str(page_size)})
    try:
        return json.loads(payload)
    except json.JSONDecodeError as exc:
        raise RuntimeError("Marketplace returned an invalid response.") from exc
...
def guest_inspect(slug: str, version: str, *, locale: str = "") -> dict[str, Any]:
    payload = _guest_request(f"/api/live/guest/strategies/{slug}/{version}", {"locale": locale})
    try:
        return json.loads(payload)
    except json.JSONDecodeError as exc:
        raise RuntimeError("Marketplace returned an invalid response.") from exc
...
def guest_versions(slug: str) -> dict[str, Any]:
    payload = _guest_request(f"/api/live/guest/strategies/{slug}/versions")
    try:
        return json.loads(payload)
    except json.JSONDecodeError as exc:
        raise RuntimeError("Marketplace returned an invalid response.") from exc
...
def guest_recommend(questionnaire: dict[str, Any]) -> dict[str, Any]:
    """Return public Live recommendations without requiring an EdgePilot account."""
    payload = _guest_request(
        "/api/live/guest/recommendations", method="POST", payload=questionnaire, structured_errors=True,
    )
    try:
        result = json.loads(payload)
    except json.JSONDecodeError as exc:
