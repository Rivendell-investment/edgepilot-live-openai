"""Compatibility-oriented orchestration for the Live Identity components."""
...
from edgepilot.platform.paths import activate_account, deactivate_account, state_root, strategies_state_root
from edgepilot.identity import client as identity_client
from edgepilot.identity.errors import AuthError
from edgepilot.identity import installation_outbox
from edgepilot.identity import credential_store
from edgepilot.identity import dashboard_login
from edgepilot.identity import session as identity_session
...
_DASHBOARD_FLOW_LOCK = dashboard_login.FLOW_LOCK
_DASHBOARD_START_LOCK = dashboard_login.START_LOCK
_DASHBOARD_FLOWS = dashboard_login.FLOWS
...
def _keyring_user(user_id: object = None) -> str:
    return credential_store.keyring_user(marketplace_origin(), user_id)


def _credential_store_error(stage: str, exc: BaseException | None = None) -> AuthError:
    return credential_store.credential_store_error(stage, exc)
...
def _read_credential_document_unlocked() -> dict[str, Any] | None:
    path, _ = _paths()
    _validate_file(path)
    if not path.exists():
        return None
...
def _read_metadata_unlocked() -> dict[str, Any] | None:
    """Read validated account metadata without returning a refresh token."""
    value = _read_credential_document_unlocked()
    if value is None:
        return None
    return {key: item for key, item in value.items() if key != "refresh_token"}
...
def _read_unlocked() -> dict[str, Any] | None:
    ...
    if value is None:
        return None
    if value.get("storage") == "keyring":
        keyring_user = value["keyring_user"]
        try:
...
def pending_installation_counts(user_id: str | None = None) -> dict[str, int]:
    return installation_outbox.pending_counts(
        user_id,
        lock=_credential_lock,
        read_items=_read_pending_unlocked,
    )


def pending_installation_issues(user_id: str) -> list[dict[str, Any]]:
    return installation_outbox.pending_issues(
        user_id,
        lock=_credential_lock,
        read_items=_read_pending_unlocked,
    )
...
def clear_pending_installations(user_id: str, strategy_slug: str) -> None:
    installation_outbox.clear_pending(
        user_id,
        strategy_slug,
        lock=_credential_lock,
        read_items=_read_pending_unlocked,
...
def prepare_installation(user_id: str, slug: str, version: str, installed_at: str) -> str:
    return installation_outbox.prepare(
        user_id,
        slug,
        version,
        installed_at,
...
def reconcile_prepared_installations(user_id: str | None = None) -> None:
    installation_outbox.reconcile_prepared(
        user_id,
        strategies_state_root(),
        lock=_credential_lock,
        read_items=_read_pending_unlocked,
        write_items=_write_pending_unlocked,
    )
...
def _request(path: str, *, method: str = "GET", payload: dict[str, Any] | None = None, token: str | None = None,
    ...
    return identity_client.request(
        marketplace_origin(),
        path,
        urlopen=urlopen,
        method=method,
        ...
        max_error_bytes=MAX_AUTH_ERROR_BYTES,
    )
...
def authenticated_request(path: str, *, method: str = "GET", payload: dict[str, Any] | None = None,
                          idempotency_key: str | None = None, timeout: int = 20) -> tuple[dict[str, Any], dict[str, str]]:
    return identity_session.authenticated_request(
        path,
        method=method,
        payload=payload,
        idempotency_key=idempotency_key,
...
def _dashboard_flow(login_id: str) -> dict[str, Any]:
    return dashboard_login.flow(login_id)


def dashboard_login_active() -> bool:
    return dashboard_login.login_active()
...
def dashboard_login_start() -> dict[str, Any]:
    with _DASHBOARD_START_LOCK:
        with _DASHBOARD_FLOW_LOCK:
            previous_flows = list(_DASHBOARD_FLOWS.values())
            _DASHBOARD_FLOWS.clear()
        for previous in previous_flows:
            try:
                _request("/api/auth/device/cancel", method="POST", payload={"device_code": previous["device_code"]})
            except AuthError as exc:
                if exc.code not in {"access_denied", "expired_token", "HTTP_404"}:
                    raise
        try:
...
def _save_dashboard_tokens(login_id: str, tokens: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        with _credential_lock():
            previous = _read_metadata_unlocked()
            if previous:
                previous_user = previous.get("user")
                previous_user_id = previous_user.get("id") if isinstance(previous_user, dict) else None
                if isinstance(previous_user_id, int):
...
def dashboard_google_open(login_id: str, handoff_url: str) -> dict[str, Any]:
    ...
    opened = bool(webbrowser.open(f"{handoff_url.rstrip('/')}/{handoff_token}", new=2))
    if not opened:
        with _DASHBOARD_FLOW_LOCK:
            current = _DASHBOARD_FLOWS.get(login_id)
