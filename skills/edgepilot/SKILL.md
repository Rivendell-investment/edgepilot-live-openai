...
Before the first preference question, call the read-only `verify_activation` tool. If it reports `activation: "ready"` and the opening request asks to open the Dashboard, call `open_dashboard` immediately, present the verified local Dashboard link or opening result, and ask the first unanswered preference question in the same reply. If the opening request does not ask for the Dashboard, ask the first question without opening it. If `verify_activation` reports `restart_required`, explain that a verified Dashboard from another plugin version is still running and ask the user to restart Codex, then continue in a new task. Never infer activation from a PID, process name, port scan, or plugin files alone.
...
When the onboarding request also asks to open the Dashboard, the original request is explicit authorization for this read-only local action. Open it immediately after activation succeeds; do not wait for questionnaire completion, confirmation, or recommendation cards. Use only the verified URL returned by the current MCP, and do not ask the user to send another Dashboard command. If the host blocks automatic navigation, present that verified URL as the single fallback link. Continue the existing one-question-at-a-time recommendation flow in the same reply. Do not install or check the native runtime merely to open the Dashboard.
