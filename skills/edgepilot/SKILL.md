...
When the onboarding request also asks to open the Dashboard, do not start it while the questionnaire is incomplete. After `recommend_strategies` successfully renders the three cards, call `open_dashboard` directly; the original onboarding request is explicit authorization for this read-only local action. Use only the verified URL returned by the current MCP, and do not ask the user to send another Dashboard command. If the host blocks automatic navigation, present that verified URL as the single fallback link. Do not install or check the native runtime merely to open the Dashboard.
