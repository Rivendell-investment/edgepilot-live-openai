...
Before the first preference question, call the read-only `verify_activation` tool. If it reports `activation: "ready"` and the onboarding request asks to open the Dashboard, call `open_dashboard` once before asking the first preference question, present only the verified URL returned by the current MCP, and then ask the first question in the same reply. A failed first Dashboard call does not invalidate or block the questionnaire; report that failure without claiming the Dashboard opened and continue with the first question. If activation reports `ready` without a Dashboard request, ask the first question directly. If it reports `restart_required`, do not call `open_dashboard`; explain that a verified Dashboard from another plugin version is still running and ask the user to restart Codex, then continue in a new task. Never infer activation from a PID, process name, port scan, or plugin files alone.
...
When the onboarding request also asks to open the Dashboard, the original request is explicit authorization for both read-only local calls. After `recommend_strategies` successfully renders the three cards, call `open_dashboard` again even if the first call succeeded, and present only the newly verified URL returned by that second call. Do not ask the user to send another Dashboard command. If the host blocks automatic navigation after either call, present that call's verified URL as the fallback link. Keep recommendation success separate from Dashboard failure, and never claim that either Dashboard call succeeded without a returned URL. Do not install or check the native runtime merely to open the Dashboard.
